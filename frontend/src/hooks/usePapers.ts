...
export function usePapers(examId: number) {
  ...
  const fetchPaper = useCallback(async (paperId: number): Promise<Paper | null> => {
    const token = getToken();
    if (!token) return null;
    try {
      return await paperService.get(examId, paperId, token);
    } catch {
      return null;
    }
  }, [examId]);
  ...
  return { papers, isLoading, error, fetchPapers, fetchPaper, deletePaper };
}